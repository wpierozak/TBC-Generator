...
void Nucleator::nucleate(Domain& domain, _long_int y0, _int g0, Layer& msp)
{
    ...
    _long_int dX = (dimX + sqrt(msp.grainsNumber) - 1)/ceil(sqrt(msp.grainsNumber));
    _long_int dZ = (dimZ + sqrt(msp.grainsNumber) - 1)/floor(sqrt(msp.grainsNumber));

    for(_long_int z = 0; z < dimZ; z+=dZ)
        for(_long_int x = 0; x < dimX; x+=dX)
        {
            ...
            m_grains[grain_ID].growth_tensor = msp.prefered_orientation;
            grain_ID++;
